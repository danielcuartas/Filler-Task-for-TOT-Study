...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    trials = data.TrialHandler2(
        ...
        extraInfo=expInfo, 
        originPath=-1, 
        trialList=data.importConditions('loop_us_state_trials.xlsx'), 
        seed=None, 
    )
    ...
    for thisTrial in trials:
        ...
        State_Naming_Task = data.Routine(
            ...
            components=[stimuli_prompt, state_pics, textbox],
        )
        ...
        while continueRoutine and routineTimer.getTime() < 8.0:
            ...
            if stimuli_prompt.status == STARTED:
                ...
                if tThisFlipGlobal > stimuli_prompt.tStartRefresh + 7.5-frameTolerance:
            ...
            # if state_pics is starting this frame...
            if state_pics.status == NOT_STARTED and tThisFlip >= 0.5-frameTolerance:
                # keep track of start time/frame for later
                state_pics.frameNStart = frameN  # exact frame index
                state_pics.tStart = t  # local t and not account for scr refresh
                state_pics.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(state_pics, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'state_pics.started')
                # update status
                state_pics.status = STARTED
                state_pics.setAutoDraw(True)
            
            # if state_pics is active this frame...
            if state_pics.status == STARTED:
                # update params
                pass
            
            # if state_pics is stopping this frame...
            if state_pics.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > state_pics.tStartRefresh + 7.5-frameTolerance:
                    # keep track of stop time/frame for later
                    state_pics.tStop = t  # not accounting for scr refresh
                    state_pics.tStopRefresh = tThisFlipGlobal  # on global time
                    state_pics.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'state_pics.stopped')
                    # update status
                    state_pics.status = FINISHED
                    state_pics.setAutoDraw(False)
            ...
            if textbox.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > textbox.tStartRefresh + 7.5-frameTolerance:
                    # keep track of stop time/frame for later
        ...
        if State_Naming_Task.maxDurationReached:
            routineTimer.addTime(-State_Naming_Task.maxDuration)
        elif State_Naming_Task.forceEnded:
            routineTimer.reset()
        else:
